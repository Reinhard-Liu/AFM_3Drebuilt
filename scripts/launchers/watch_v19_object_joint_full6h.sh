...
RUNNER="$ROOT/scripts/launchers/run_v19_object_joint_full6h.sh"
TRAIN_DIR="$ROOT/experiments/v19_object_joint_full6h"
SAVE_DIR="$TRAIN_DIR/checkpoints"
LOG_DIR="$TRAIN_DIR/logs"
WATCH_LOG="$LOG_DIR/watchdog.log"
CONFIG="$ROOT/configs/config_v19_object_joint_full6h.json"
...
while true; do
  mapfile -t pids < <(pgrep -f "python3 -u -m src.train_v19_object_joint --config_full6h.json" || true)
  if [[ "${#pids[@]}" -eq 0 ]]; then
