...
RUNNER="$ROOT/run_v19_object_joint_full6h.sh"
TRAIN_DIR="$ROOT/experiments/v19_object_joint_full6h"
SAVE_DIR="$TRAIN_DIR/checkpoints"
LOG_DIR="$TRAIN_DIR/logs"
WATCH_LOG="$LOG_DIR/watchdog.log"
CONFIG="$ROOT/config_v19_object_joint_full6h.json"
...
p = Path("/root/autodl-tmp/micro/config_v19_object_joint_full6h.json")
...
from pathlib import Path
import torch
p = Path("/root/autodl-tmp/micro/experiments/v19_object_joint_full6h/checkpoints/latest_v19_object_joint.pt")
if not p.exists():
    print(-1)
else:
...
while true; do
  mapfile -t pids < <(pgrep -f "python3 -u -m src.train_v19_object_joint --config /root/autodl-tmp/micro/config_v19_object_joint_full6h.json" || true)
  if [[ "${#pids[@]}" -eq 0 ]]; then
