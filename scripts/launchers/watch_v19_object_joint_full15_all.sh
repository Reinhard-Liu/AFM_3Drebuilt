...
RUNNER="$ROOT/scripts/launchers/run_v19_object_joint_full15_all.sh"
...
while true; do
  mapfile -t pids < <(pgrep -f "python3 -u -m src.train_v19_object_joint --config_full15_all.json" || true)
  if [[ "${#pids[@]}" -eq 0 ]]; then
